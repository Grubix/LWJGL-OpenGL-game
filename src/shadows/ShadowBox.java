...
public class ShadowBox {
	...
	private float farPlaneHeight, farPlaneWidth;
	 ...
	 */
	protected void update() { //TODO tutaj jest problem !!!!!!!!!!!!!!
		Matrix4f rotation = calculateCameraRotationMatrix();
		Vector4f tmp = rotation.transform(FORWARD); //TODO ZMIENIC NAZWE
		Vector3f forwardVector = new Vector3f(tmp.x, tmp.y, tmp.z);

		Vector3f toFar = new Vector3f(forwardVector).mul(SHADOW_DISTANCE);
		Vector3f centerFar = new Vector3f(toFar).add(camera.getPosition());
		
		Vector3f toNear = new Vector3f(forwardVector).mul(camera.getNearPlane());
		Vector3f centerNear = new Vector3f(toNear).add(camera.getPosition());
		...
		//System.out.println("width: " + getWidth() + " | height: " + getHeight() + "| length: " + getLength());
	}
	...
	protected Vector3f getCenter() {
		...
		center = invertedLight.transform(center);
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
		...
		Vector4f upVector4f = new Matrix4f(rotation).transform(UP);
		...
		Vector3f farTop = new Vector3f(centerFar).
				add(new Vector3f(upVector.x * farPlaneHeight, upVector.y * farPlaneHeight, upVector.z * farPlaneHeight));
		Vector3f farBottom = new Vector3f(centerFar).
				add(new Vector3f(downVector.x * farPlaneHeight, downVector.y * farPlaneHeight, downVector.z * farPlaneHeight));
		Vector3f nearTop = new Vector3f(centerNear).
				add(new Vector3f(upVector.x * nearPlaneHeight, upVector.y * nearPlaneHeight, upVector.z * nearPlaneHeight));
		Vector3f nearBottom = new Vector3f(centerNear).
				add(new Vector3f(downVector.x * nearPlaneHeight, downVector.y * nearPlaneHeight, downVector.z * nearPlaneHeight));

		Vector4f[] points = new Vector4f[8];
		points[0] = calculateLightSpaceFrustumCorner(farTop, rightVector, farPlaneWidth);
		points[1] = calculateLightSpaceFrustumCorner(farTop, leftVector, farPlaneWidth);
		points[2] = calculateLightSpaceFrustumCorner(farBottom, rightVector, farPlaneWidth);
		points[3] = calculateLightSpaceFrustumCorner(farBottom, leftVector, farPlaneWidth);
		points[4] = calculateLightSpaceFrustumCorner(nearTop, rightVector, nearPlaneWidth);
		points[5] = calculateLightSpaceFrustumCorner(nearTop, leftVector, nearPlaneWidth);
		points[6] = calculateLightSpaceFrustumCorner(nearBottom, rightVector, nearPlaneWidth);
		points[7] = calculateLightSpaceFrustumCorner(nearBottom, leftVector, nearPlaneWidth);
	...
	private Vector4f calculateLightSpaceFrustumCorner(Vector3f startPoint, Vector3f direction, float width) {
		Vector3f point = new Vector3f(startPoint).
				add(new Vector3f(direction.x * width, direction.y * width, direction.z * width));
		Vector4f point4f = new Vector4f(point, 1f);

		return new Matrix4f(lightViewMatrix).transform(point4f);
	}
...
//		rotation.rotate((float) Math.toRadians(-cameraRotation.y), new Vector3f(0, 1, 0));
	...
	private void calculateWidthsAndHeights() {
		farPlaneWidth = (float) (2 * SHADOW_DISTANCE * Math.tan(Math.toRadians(camera.getFov() / 2)));
		nearPlaneWidth = (float) (2 * camera.getNearPlane() * Math.tan(Math.toRadians(camera.getFov() / 2)));
		farPlaneHeight = farPlaneWidth / getAspectRatio();
		nearPlaneHeight = nearPlaneWidth / getAspectRatio();
		
		System.out.println("farWidth: " + farPlaneWidth + " | FarHeight: " + farPlaneHeight + "| nearWidth: " + nearPlaneWidth + "| nearHeight: " + nearPlaneHeight);
	}
	...
	private float getAspectRatio() {
		return Main.getWindowWidth() / Main.getWindowHeight();
	}
